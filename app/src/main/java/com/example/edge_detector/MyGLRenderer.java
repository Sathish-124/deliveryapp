    ...
    public void onDrawFrame(GL10 gl) {
        ...
        synchronized (mImageLock) {
            if (mImageDirty && mImageData != null) {
                ...
                // Assuming RGBA for now, or GRAYSCALE depending on what we send
                // If sending GRAYSCALE (Luminance), use GL_LUMINANCE
                // Let's assume we send RGBA from JNI for compatibility or GRAYSCALE
                // If we simply run Canny, we get 1 channel.
                // Let's stick to GL_LUMINANCE for simplicity if output is grayscale.
